...
class DescargarProgramaForm extends FormBase
{
  ...
  public function submitForm(array &$form, FormStateInterface $form_state)
  {
    ...
    if (
      ...
      $programa->bundle() === 'programa' &&
      $programa->hasField('field_correo_descarga') &&
      !$programa->get('field_correo_descarga')->isEmpty()
    ) {
      $mail_config_key = trim((string) $programa->get('field_correo_descarga')->value);
    }
    ...
    if ($subject_config === '') {
      $subject_config = 'Descarga programa - [programa]';
    }

    $copy_subject_config = trim((string) ($config_email['copy_subject'] ?? ''));
    if ($copy_subject_config === '') {
      $copy_subject_config = 'Notificación descarga programa - [programa]';
    }
